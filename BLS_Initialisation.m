% BLS_INITIALISATION  Initialise everything for the BLS algorithm
%
% [LIKELIHOOD, BASIS, BASISSCALES, ALPHA, BETA, MU, PHI, USED] = ...
%    BLS_INITIALISATION(TYPE, BASIS, TARGETS, SETTINGS, OPTIONS)
%
% OUTPUT ARGUMENTS:
% 
%	LIKELIHOOD	Likelihood structure (from BLS_LIKELIHOODS)
...
%				BLS_PARAMETERSETTINGS 
% 
%	OPTIONS		User options structure from BLS_USEROPTIONS
%	
% NOTES: 
% 
% This function initialises all necessary model parameters appropriately
% before entering the main loop of the BLS inference algorithm.
%
% This function is intended for internal use by BLS only.
%
...
% 
[BASIS, BasisScales] = BLS_PreProcessBasis(BASIS);
...
% functions (BLS_PARAMETERSETTINGS) and any "free" ones (BLS_USEROPTIONS).
...
if isempty(SETTINGS.Mu)
  ...
  BLS_Diagnostic(OPTIONS, 2,'Initialising with supplied weights\n');
...
if isempty(SETTINGS.Alpha)
  ...
  if M==1
    BLS_Diagnostic(OPTIONS,2,'Initial alpha = %g\n', Alpha);
  end
else
