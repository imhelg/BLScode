% BLS: main estimation algorithm
%
% [PARAMETER, HYPERPARAMETER, DIAGNOSTIC] = ...
%   BLS(LIKELIHOOD, BASIS, TARGETS, OPTIONS, SETTINGS)
%
% OUTPUT ARGUMENTS:
% 
...
% 
%	OPTIONS		User-specified settings via BLS_USEROPTIONS [Optional]
% 
%	SETTINGS	Initialisation of main parameter values via
%				BLS_PARAMETERSETTINGS [Optional]
%
% NOTES: 
%
% 
% Given inputs (BASIS) and desired outputs (TARGETS) BLS will optimise the
% log-marginal-likelihood of the corresponding sparse Bayesian model and 
% should return (given reasonable choice of basis) a sparse vector of model 
% parameters.
% 
...
% SEE ALSO:
%
%	BLS_USEROPTIONS: available user-definable options, such as the
%	number of iterations to run for, level of diagnostic output etc.
% 
%	BLS_PARAMETERSETTINGS: facility to change default initial values for
%	parameters and hyperparameters.
% 
%	BLS_CONTROLSETTINGS: hard-wired internal algorithm settings.
%
% The main algorithm of the Bayesian Lasso Sparse (BLS) is based upon that
% outlined in "A Bayesian Lasso based Sparse Learning Model", by
% Helgøy & Li, submitted to Computational statistics and data analysis.
%
% The implementation in this file is based on the RVM-implementation in
% the toolbox SPARSEBAYES library for Matlab (V2.0) and contains the
% the necessary changes to translate the algorithm from RVM to BLS.
...
function [PARAMETER, HYPERPARAMETER, DIAGNOSTIC] = ...
    BLS(likelihood_, BASIS, Targets, OPTIONS, SETTINGS)
...
if ~exist('SETTINGS','var')   % DEFAULT SET TO ZERO
  SETTINGS	= BLS_ParameterSettings;
end

%% If no user options passed, import defaults
if ~exist('OPTIONS','var')
  OPTIONS	= BLS_UserOptions;
end
...
[LIKELIHOOD, BASIS, BasisScales, Alpha, beta, Mu, PHI, Used] = ...
    BLS_Initialisation(likelihood_, BASIS, Targets, SETTINGS, OPTIONS);
%
...
lambda=2*(length(Used) -1)/sum(1./Alpha);

[SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,BASIS_B_PHI,beta] = ...
      BLS_FullStatistics(BASIS,PHI,Targets,Used, ...
			 Alpha,beta,Mu,BASIS_PHI,BASIS_Targets,lambda,OPTIONS); 
...
while (~LAST_ITERATION)
  ...
  %
  Action		= ACTION_REESTIMATE*ones(M_full,1); % Default   zero first round
  %
  ...
  UsedFactor	= Factor(Used);
   ...
   lambda       = 2*(length(Used) -1)/sum(1./Alpha);
   AA           = lambda*beta + S_out - Q_out.^2;
   BB           = 2*lambda*beta.*S_out + S_out.^2;
   CC           = lambda.*S_out.^2;
   
   DISC         = BB.^2 - (4*beta).*AA.*CC;
   nextAlphas   = (-BB-sqrt(DISC))./(2*beta*AA);
  ...
  % 
  iu		= UsedFactor>max([(lambda*beta) CONTROLS.ZeroFactor]);      %CONTROLS.ZeroFactor;
  index		= Used(iu);
  NewAlpha	= nextAlphas(index);  %S_out(index).^2 ./ Factor(index);

  %
  % Quick computation of change in log-likelihood given all re-estimations
  % 
  DeltaML(index)	= Q_in(index).^2./(beta.*NewAlpha + S_in(index)) ...
                      + log((beta.*NewAlpha)./(beta.*NewAlpha + S_in(index))) - lambda./NewAlpha ...
                      - Q_in(index).^2./(beta.*Alpha(iu) + S_in(index)) ...
                      - log((beta.*Alpha(iu))./(beta.*Alpha(iu) + S_in(index))) + lambda./Alpha(iu);
  ...
  if anyToDelete
	...
	% 
    DeltaML(index)	= -(Q_out(index).^2 ./ (S_out(index) + beta.*Alpha(iu))) - ...
            log((beta.*Alpha(iu))./(beta.*Alpha(iu) + S_out(index)))+ lambda./Alpha(iu);
  ...
  % 
  % ADDITION: must be a POSITIVE factor and OUT of the model % factor =
  % Q^2-S
  % 
  % Find ALL good factors ...
  GoodFactor		= Factor> max([beta*lambda CONTROLS.ZeroFactor]);
  ...
  if anyToAdd
	...
	% 
     NAlp			= nextAlphas(index);    
    DeltaML(index)	= log((beta.*NAlp)./(beta.*NAlp + S_in(index))) ...
                      + Q_in(index).^2./(beta.*NAlp + S_in(index)) ...
                       - lambda./NAlp ; 
  ...
  if CONTROLS.BasisAlignmentTest
    ...
    if selectedAction==ACTION_DELETE
      ...
      if numAligned>0
		...
		BLS_Diagnostic(OPTIONS,3,'Alignment reinstatement of %s\n', r_);
      end
    end
  end
  ...
  switch selectedAction
   ...
   case ACTION_REESTIMATE
    ...
    kappa		= 1/(SIGMA(j,j)+((1/beta).*deltaInv));
    tmp			= kappa*s_j;
  ...
  M		= length(Used);
  %
  BLS_Diagnostic(OPTIONS, 3, 'ACTION: %s of %d (%g)\n', ...
  ...
  if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian && ...
    ...
    beta_temp  = beta;
    diagAbeta  = diag(1./Alpha);
    C_beta     = chol(eye(N) + PHI*diagAbeta*PHI');
    IDLL       = inv(C_beta);
    beta       = N/(Targets'*(IDLL*IDLL')*Targets);


    % Work-around zero-noise issue
    beta	= min([beta CONTROLS.BetaMaxFactor/var(Targets)]);
    %
    deltaLogBeta  = log(beta)-log(beta_temp);
    ...
    if abs(deltaLogBeta) > CONTROLS.MinDeltaLogBeta
      ...
      [SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,BASIS_B_PHI] = ...
		  BLS_FullStatistics(BASIS,PHI,Targets,Used,Alpha,beta,...
							 Mu, BASIS_PHI,BASIS_Targets,lambda,OPTIONS); 
      %
      count					= count + 1;
      logMarginalLog(count)	= logML;
      %
      if selectedAction==ACTION_TERMINATE
		...
		BLS_Diagnostic(OPTIONS,3,'Noise update (termination deferred)\n');
      end
    end
  end
  ...
  if (selectedAction==ACTION_TERMINATE)
    ...
    % 
    BLS_Diagnostic(OPTIONS,2,...
				   '** Stopping at iteration %d (Max_delta_ml=%g) **\n', ...
				   i, deltaLogMarginal);
    if LIKELIHOOD.InUse~=LIKELIHOOD.Gaussian
      BLS_Diagnostic(OPTIONS,2,'%4d> L = %.6f\t Gamma = %.2f (M = %d)\n',...
  ...
  if (OPTIONS.monitor && ~rem(i,OPTIONS.monitor)) || LAST_ITERATION
	...
	% 
    if LIKELIHOOD.InUse~=LIKELIHOOD.Gaussian
      BLS_Diagnostic(OPTIONS,2,'%5d> L = %.6f\t Gamma = %.2f (M = %d)\n',...
		     i, logML/N, sum(Gamma), M);
    else
      BLS_Diagnostic(OPTIONS,2,...
