% BLS_FULLSTATISTICS  Compute all relevant statistics in full for BLS
%
% [SIGMA,MU,S_IN,Q_IN,S_OUT,Q_OUT,FACTOR,LOGML,GAMMA,...
%	BETABASIS_PHI,BETA] = ...
%  BLS_FULLSTATISTICS(LIKELIHOOD,BASIS,PHI,TARGETS,USED,ALPHA,BETA,...
...
%	OPTIONS			Standard OPTIONS structure (see BLS_USEROPTIONS)
% 
% NOTES:
%
% This function computes the posterior, and other, statistics for the
% BLS algorithm in "long-hand" fashion. i.e. it does not use the
...
% Compute full relevant statistics
  ...
  % 
  U		= chol(PHI'*PHI*beta + diag(Alpha.*beta));  
  Ui	= inv(U);
  SIGMA	= Ui * Ui';
  % Posterior mean Mu
...
logML			= dataLikely - (Mu.^2)'*(beta.*Alpha)/2 + ...
    sum(log(beta.*Alpha))/2 - logdetHOver2 ...
    + N*log(lambda) - lambda/2 *sum(1./Alpha);  
...
Q_out		= Q_in;
%
% S,Q with that basis excluded:
% 
S_out(Used)	= ((beta.*Alpha) .* S_in(Used)) ./ ((beta.*Alpha) - S_in(Used));
Q_out(Used)	= ((beta.*Alpha) .* Q_in(Used)) ./ ((beta.*Alpha) - S_in(Used));
%
